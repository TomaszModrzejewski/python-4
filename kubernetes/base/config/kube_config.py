...
class CommandTokenSource(object):
    def __init__(self, cmd, args, tokenKey, expiryKey):
        self._cmd = cmd
        self._args = args
        self._tokenKey = tokenKey or '{.access_token}'
        self._expiryKey = expiryKey or '{.token_expiry}'
...
class KubeConfigLoader(object):
    ...
    def __init__(self, config_dict, active_context=None,
        ...
        self._get_google_credentials = get_google_credentials or _refresh_credentials
    ...
    def _load_azure_token(self, provider):
        ...
        if 'access-token' not in provider['config']:
            return
        if 'expires-on' in provider['config'] and self._azure_is_expired(provider):
